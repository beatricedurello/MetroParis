...
class Controller:
    ...
    def handleCercaRaggiungibili(self,e):
        if self._fermataPartenza is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(ft.Text("Attenzione, stazione di partenza non serezionata", color="red"))
            self._view.update_page()
            return
    ...
    def handleCerca(self,e):
        if self._fermataPartenza is None or self._fermataArrivo is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(ft.Text(f"Attenzione selezionare fermate di arrivo e partenza", color="red"))
            self._view.update_page()
            return
        ...
        if path == []:
            ...
            self._view.lst_result.controls.append(
                ft.Text(f"Non ho un cammino fra {self._fermataPartenza} e {self._fermataArrivo}", color="red"))
            self._view.update_page()
            return
    ...
    def loadFermate(self, dd: ft.Dropdown()):
            ...
            if dd.label == "Stazione di Partenza":
                ...
                for f in fermate:
                    dd.options.append(ft.dropdown.Option(text=f.nome, data=f, on_click=self.read_DD_Arrivo))
